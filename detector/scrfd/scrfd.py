# Code baseline from https://github.com/deepinsight/insightface/blob/master/detection/scrfd/tools/scrfd.py
...
_weights = {
    "scrfd_10g_gnkps": "https://drive.google.com/uc?id=1v9nhtPWMLSedueeL6c3nJEoIFlSNSCvh"
}
...
class SCRFD:
    def __init__(self, model_file=None, nms_thresh=None, session=None):
        if not osp.exists(model_file):
            if not osp.exists(model_file.parent):
                os.mkdir(model_file.parent)
            url = _weights[model_file.stem]
        ...
        if self.session is None:
            ...
            self.session = onnxruntime.InferenceSession(
                self.model_file, so, providers=exproviders
            )
        self.center_cache = {}
