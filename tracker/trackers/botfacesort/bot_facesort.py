...
class STrack(BaseTrack):
    ...
    shared_memory = deque([], maxlen=1000)
    ...
    def update_features(self, feat, id_, norm=True):
        if norm:
        ...
        STrack.shared_memory.append((self.smooth_feat, id_))
    ...
    def activate(self, frame_id):
        ...
        if self.curr_feat is not None:
            self.update_features(self.curr_feat, id_=self.id)
    ...
    def re_activate(self, new_track, frame_id, new_id=False, shot_changed=False):
        if shot_changed:
            self.kalman_filter = KalmanFilter()
        ...
        if new_track.curr_feat is not None:
            self.update_features(new_track.curr_feat, id_=self.id)
    ...
    def update(self, new_track, frame_id, shot_changed=False):
        """
...
class BoTFaceSORT(object):
    ...
    def update(self, face_dets, img, shot_changed, landmarks=None):
        assert isinstance(
        ...
        # if not shot_changed:
        #     warp = self.cmc.apply(img, face_dets)
        #     STrack.multi_gmc(strack_pool, warp)
        #     STrack.multi_gmc(unconfirmed, warp)

        """ Step 3. First association: Adaptive association for matched faces based on IoU and embedding distance via shot changed"""
        iou_dists = iou_distance(strack_pool, detections, method="iou")
        iou_dists_mask = iou_dists > self.proximity_thresh
        emb_dists = embedding_distance_modified(strack_pool, detections) / 2.0
        ...
        for itracked, idet in matches:
            ...
            if track.state == TrackState.Tracked:
                track.update(det, self.frame_id, shot_changed=shot_changed)
                activated_starcks.append(track)
            else:
                track.re_activate(
                    det, self.frame_id, new_id=False, shot_changed=shot_changed
                )
                refind_stracks.append(track)

        """ Step 3: Second association, with embedding distance"""
        ...
        ]  # unmatched track that are being tracked

        remain_stracks = joint_stracks(r_tracked_stracks, unconfirmed)

        emb_dists = embedding_distance_modified(remain_stracks, second_detections) / 2.0
        emb_dists[emb_dists > self.face_appearance_thresh] = 1.0
        dists = emb_dists
