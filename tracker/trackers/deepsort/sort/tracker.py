...
import numpy as np
from ....motion.kalman_filters import deepsort_kf
...
class Tracker:
    ...
    GATING_THRESHOLD = np.sqrt(deepsort_kf.chi2inv95[4])
    ...
    def __init__(self, metric, max_iou_dist=0.9, max_age=30, n_init=3, _lambda=0):
        ...
        self.kf = deepsort_kf.KalmanFilter()
        self.tracks = []
        self._next_id = 1
    ...
    def _full_cost_metric(self, tracks, dets, track_indices, detection_indices):
        ...
        app_cost = self.metric.distance(
            np.array([dets[i].feat for i in detection_indices]),
    ...
    def _initiate_track(self, detection):
        self.tracks.append(Track(detection, self._next_id, self.n_init, self.max_age))
        self._next_id += 1
