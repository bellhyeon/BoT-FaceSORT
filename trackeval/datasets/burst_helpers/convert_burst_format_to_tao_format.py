...
def main(args):
    ...
    tao_format_pred = PredictionBURSTFormatToTAOFormatConverter(
        tao_format_gt, ali_format_pred, args.split, args.exemplar_guided
    ).convert()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gt_output_file", type=str, default="/tmp/val_gt.json")
    parser.add_argument(
        "--pred_input_file",
        type=str,
        default="../data/trackers/tsunami/exemplar_guided/STCN_off_the_shelf/data/results.json",
    )
    parser.add_argument("--pred_output_file", type=str, default="/tmp/pred.json")
